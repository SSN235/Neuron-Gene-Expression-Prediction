...
- Select how many neurons to validate (1–500)
...
| **Model** | PyTorch | Trained NGEP model from 10-fold CV |
...
```python
# Validator API parameters
species = "mouse"
brain_region = "neocortex"
pagesize = 500

# Query with randomization to avoid training data
...
# Randomize neuron order to minimize overlap with training set
fetched_neurons = [random.shuffle() for neurons in response]
```

**Key Feature:** Neurons are retrieved in **random order** to minimize the likelihood of selecting neurons that were in the original training dataset (150 neurons from 300,000+ available in NeuroMorpho.org).

#### 2. **Feature Extraction**
For each fetched neuron, the validator:
- Downloads the SWC morphology file
- Extracts the 5 morphological features (soma radius, dendritic length, bifurcations, terminals, branch density)
- Applies StandardScaler normalization (using the same scaler fitted during training)

```python
features = extract_swc_features(swc_data)
...
│  (1–500; higher numbers take longer)     │
│                                         │
...
The validator minimizes overlap with the original 150 training neurons through:

1. **Large population:** NeuroMorpho.org has >300,000 neurons total; training set is <0.05%
2. **Random ordering:** Neurons fetched in random order, not sequential
3. **Statistical expectation:** With 150 training neurons and 300,000 available, probability of refetching the same neuron is ~0.05% per validation neuron
4. **Transparent reporting:** Validator notes any neurons that appear in both training and validation sets

**Expected contamination rate:** ~0.05% (negligible for statistical purposes)
...
- **Region matching:** Not performed for validation (only morphological features used)
...
- Different neurons (external validation set)
...
Validator Backend (Render)
    ↓
    Loads all 10 models, makes ensemble predictions
    ↓
...
- **Two hidden layers:** Sufficient complexity for feature interactions without overfitting on n=150
- **Batch normalization:** Stabilizes training on small dataset; accelerates convergence
...
- **Final fold size:** ~120 training samples, ~15 test samples
...
✅ **Real data throughout:** All 150 neurons from NeuroMorpho.org; all expression from Allen Brain Atlas (no synthetic data)  
...
⚠️ **Small dataset (n=150):** Larger samples would reduce variance and enable deeper architectures  
⚠️ **Sparse feature set (5 features):** Spine density, axonal morphology, electrophysiology not included  
